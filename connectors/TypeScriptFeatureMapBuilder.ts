...
${properties
    ...
    .map(([key, value]) => indentString(`${key}: ${value}`, 4))
    .join(",\n")}
}`;
...
const createFeatureString = (featureName: string, feature?: Feature): string => {
    return objectToString([
        ["$name", `"${featureName}"`],
        ...Object.keys(feature?.features || {}).map(
            featureName => [toSafeProperty(featureName), createFeatureString(featureName, feature?.features?.[featureName])] as [string, string]
        )
    ]);
};
...
${createConstantString(
    `${featureMap.productName}FeatureMap`,
    Object.keys(featureMap.features).map(featureName => [toSafeProperty(featureName), createFeatureString(featureName, featureMap.features[featureName])])
)}`;
};
